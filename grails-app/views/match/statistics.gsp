...
<table class="normal">
	<thead>
		<tr>
			...
			<g:each in="${players}" status="i" var="player">
				<th><g:link controller="player" action="show" id="${player}"> 
			${Player.get(player)?.shortName()}</g:link></th>
			</g:each>
			<th>Name</th>
		</tr>
	</thead>
	<tbody>
		<g:each in="${partnerArray}" status="x" var="pline">
			<tr>
				<td><b> <g:link controller="player" action="show"
					id="${players[x]}"> 
			${Player.get(players[x])?.shortName()}</g:link></b></td>
		...
		<table class="normal">
			<g:each in="${keys.reverse()}" status="i" var="key">
				<g:if test="${partnersScore[key].toInteger()>0 && 5 > i}">
					<tr>
						<th><g:set var="pids" value="${key.tokenize(':')}" /> <g:link
							controller="player" action="show" id="${pids[0]}"> 
			${Player.get(pids[0]).name}</g:link></th>
						<th><g:link controller="player" action="show" id="${pids[1]}"> 
			${Player.get(pids[1]).name}</g:link></th>
